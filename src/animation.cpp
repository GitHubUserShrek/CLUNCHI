...
void Animation::update(Mood currentMood)
{
    ...
    else if (now > lastBlink_ && !suppressBlink)
    {
        isBlinking_ = true;
        blinkPhase_ = 0;
    }
    ...
    if (showSpiralEyes_ || (isAttackAnimActive_ && currentAttackType_ == ATTACK_BEACON_FLOOD))
    {
    ...
    {
        if (currentAttackType_ == ATTACK_HANDSHAKE_CAPTURE)
        {
            pupilX_ = ((now % 800) < 400) ? -5 : 5;
            pupilY_ = 0;
        }
        else if (currentAttackType_ == ATTACK_CTS_JAMMING)
        {
            pupilX_ = 0;
            pupilY_ = 2;
        }
        else if (currentAttackType_ == ATTACK_EVIL_TWIN)
        {
            pupilX_ = (int)(sin(now / 300.0f) * 2.0f);
            pupilY_ = 0;
        }
        else
        {
    ...
    {
        if (currentAttackType_ == ATTACK_CTS_JAMMING)
            bounceY_ = sin(now / 20.0f) * 1.5f;
        else if (currentAttackType_ == ATTACK_EVIL_TWIN)
            bounceY_ = sin(now / 200.0f) * 1.0f;
        else
...
void Animation::triggerAttackFace(AttackType type, uint32_t duration)
{
    ...
    attackFaceEndTime_ = millis() + duration;
    showHeartEyes_ = false;
    showSpiralEyes_ = (type == ATTACK_BEACON_FLOOD);
}
