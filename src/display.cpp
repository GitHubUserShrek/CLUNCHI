...
void Display::drawAttackFace(AnimState anim)
{
    ...
    case ATTACK_EVIL_TWIN:
    {
        ...
        if (seamFlicker)
        {
            for (int y = 14; y < 52; y += 3)
            {
                ...
                u8g2_.drawVLine(mouthCX + jitter, y, 2);
            }
            u8g2_.drawPixel(mouthCX + 4, 18 + (int)((now / 70) % 6));
            u8g2_.drawPixel(mouthCX - 4, 40 - (int)((now / 60) % 6));
        }
        ...
        if ((now / 400) % 2)
        {
            int spkX = leftX - 16, spkY = eyeYLeft - 11;
            u8g2_.drawPixel(spkX, spkY);
            u8g2_.drawPixel(spkX - 1, spkY + 1);
            u8g2_.drawPixel(spkX + 1, spkY + 1);
            u8g2_.drawPixel(spkX, spkY + 2);
        }

        u8g2_.drawCircle(mouthCX, mouthY - 8, 12, U8G2_DRAW_LOWER_LEFT);

        u8g2_.drawTriangle(rightX - 7, eyeYRight, rightX + 7, eyeYRight, rightX, eyeYRight - 7);
        u8g2_.drawTriangle(rightX - 7, eyeYRight, rightX + 7, eyeYRight, rightX, eyeYRight + 7);
        u8g2_.setDrawColor(0);
        int impPupil = ((now / 200) % 2) ? 2 : 1;
        u8g2_.drawDisc(rightX + anim.pupilX, eyeYRight + anim.pupilY, impPupil);
        u8g2_.setDrawColor(1);
        if ((now / 120) % 3 == 0)
            u8g2_.drawHLine(rightX - 6, eyeYRight - 1, 12);
        u8g2_.drawLine(rightX + 8, eyeYRight - 13, rightX - 4, eyeYRight - 8);
        u8g2_.drawLine(rightX + 8, eyeYRight - 12, rightX - 4, eyeYRight - 7);

        float grinCycle = fmod(now / 2200.0f, 1.0f);
        int grinLift = 2 + (int)(sin(grinCycle * PI) * 3.0f);
        int rMouthY = mouthY + 2;

        int gStartX = mouthCX;
        int gEndX = mouthCX + 16;
        ...
        for (int i = 1; i <= 8; i++)
        {
            ...
            u8g2_.drawLine(prevX, prevY, x, y);
            prevX = x;
            prevY = y;
        }
        ...
        u8g2_.drawTriangle(cornerX - 4, cornerY + 2,
                           cornerX, cornerY + 2,
                           cornerX - 2, cornerY + 6);
    ...
    {
        u8g2_.drawDisc(leftX, eyeYLeft, eyeR + 1);
        u8g2_.drawDisc(rightX, eyeYRight, eyeR + 1);
        u8g2_.setDrawColor(0);
        u8g2_.drawDisc(leftX + anim.pupilX, eyeYLeft + anim.pupilY, 2);
        u8g2_.drawDisc(rightX + anim.pupilX, eyeYRight + anim.pupilY, 2);
        u8g2_.setDrawColor(1);
        u8g2_.drawBox(mouthCX - 12, mouthY - 4, 24, 8);
        u8g2_.setDrawColor(0);
        for (int x = mouthCX - 10; x < mouthCX + 10; x += 5)
            u8g2_.drawLine(x, mouthY - 4, x + 3, mouthY + 3);
        u8g2_.setDrawColor(1);
    ...
    case ATTACK_HANDSHAKE_CAPTURE:
    {
        ...
        u8g2_.drawLine(mouthCX - 8, mouthY + wave, mouthCX - 4, mouthY - wave);
        u8g2_.drawLine(mouthCX - 4, mouthY - wave, mouthCX, mouthY + wave);
        u8g2_.drawLine(mouthCX, mouthY + wave, mouthCX + 4, mouthY - wave);
        u8g2_.drawLine(mouthCX + 4, mouthY - wave, mouthCX + 8, mouthY + wave);
        int lx = 112, ly = 48;
        u8g2_.drawFrame(lx, ly + 4, 8, 7);
        u8g2_.drawCircle(lx + 4, ly + 4, 3, U8G2_DRAW_UPPER_LEFT);
        u8g2_.drawVLine(lx + 7, ly + 1, 3);
    ...
    case ATTACK_BEACON_FLOOD:
    {
        ...
        u8g2_.drawCircle(mouthCX, mbot, mhalf, U8G2_DRAW_UPPER_LEFT | U8G2_DRAW_UPPER_RIGHT);
        u8g2_.drawHLine(mouthCX - mhalf, mbot, mhalf * 2 + 1);

        int tongueSway = (int)(sin(now / 350.0f) * 2.0f);
        int tx = mouthCX + 2 + tongueSway;
        u8g2_.drawDisc(tx, mbot + 2, 2);

        int streakL_X = mouthCX - mhalf + 2;
        int streakR_X = mouthCX + mhalf - 2;
    ...
    case ATTACK_DISASSOC:
    {
        ...
        u8g2_.drawDisc(rightX, eyeYRight, eyeR + 1);
        u8g2_.setDrawColor(0);
        u8g2_.drawDisc(leftX - 2, eyeYLeft - 2, 2);
        u8g2_.drawDisc(rightX - 2, eyeYRight - 2, 2);
        u8g2_.setDrawColor(1);

        u8g2_.drawLine(leftX - 8, eyeYLeft - 11, leftX + 6, eyeYLeft - 14);
        u8g2_.drawLine(rightX - 6, eyeYRight - 14, rightX + 8, eyeYRight - 11);

        uint32_t tearTime = millis() / 60;
        int tearY1 = (tearTime) % 16;
        int tearY2 = (tearTime + 8) % 16;

        u8g2_.drawVLine(leftX - 5, eyeYLeft + 6 + tearY1, 4);
        u8g2_.drawVLine(leftX - 2, eyeYLeft + 8 + tearY2, 3);
        u8g2_.drawVLine(rightX + 5, eyeYRight + 6 + tearY1, 4);
        u8g2_.drawVLine(rightX + 2, eyeYRight + 8 + tearY2, 3);

        u8g2_.drawHLine(mouthCX - 6, mouthY, 12);
        u8g2_.drawLine(mouthCX - 10, mouthY + 3, mouthCX - 6, mouthY);
        u8g2_.drawLine(mouthCX + 6, mouthY, mouthCX + 10, mouthY + 3);
...
void Display::drawFace(Mood currentMood, AnimState anim)
{
    ...
    else if (currentMood == TACTICAL)
    {
        ...
        else
        {
            ...
            u8g2_.drawHLine(mouthCX - 6, mouthY, 12);
            u8g2_.drawPixel(mouthCX - 7, mouthY - 1);
            u8g2_.drawPixel(mouthCX + 6, mouthY - 1);
        }
    }
