...
void Magic8Ball::updateAndDraw(bool tiltMode)
{
    ...
    {
        drawBallShape(CX, CY);
        if (quote_ && strlen(quote_) > 0)
            drawAnswerWindow(CX, CY, quote_);
    }
...
void Magic8Ball::drawAnswerWindow(int cx, int cy, const char *quote)
{
    ...
    {
        String l1 = qStr.substring(0, spaceIdx);
        String l2 = qStr.substring(spaceIdx + 1);
    ...
    {
        int w = u8g2_.getStrWidth(quote);
        u8g2_.drawStr(winX - (w / 2), winY + 3, quote);
    }
}