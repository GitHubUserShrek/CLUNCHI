...
static const char* ALPR_SSID_PATTERNS[] = {
    // Flock
    "flock", "FS Ext Battery", "Penguin", "Pigvision", "FalconLPR", "FS-",
    // Motorola/Vigilant
    "vigilant", "LEARN-", "avigilon", "PIPS", "MotoALPR",
    // Axon
    "AxonFleet", "axon-", "Fleet3",
    // Genetec
    "AutoVu", "genetec",
    // Hikvision
    "HIK-", "Hikvision", "DS-",
...
static uint16_t detectFlockBLE(const uint8_t* payload, size_t len, 
    ...
    if (!payload || len < 4) return ALPR_FLAG_NONE;
...
public:
    void onResult(const NimBLEAdvertisedDevice* device) override {
        ...
        if (bleFlags == ALPR_FLAG_NONE) return;
...
ALPRVendor alprIdentifyVendor(const uint8_t* mac) {
    if (!mac) return VENDOR_UNKNOWN;
    if (mac[0] & 0x02) return VENDOR_UNKNOWN;
...
ALPRDeviceType alprGuessDeviceType(ALPRVendor v, const char* ssid) {
    ...
    switch (v) {
        case VENDOR_FLOCK:      return TYPE_ALPR;
        case VENDOR_MOTOROLA:   return TYPE_ALPR;
        case VENDOR_AXON:       return TYPE_BODY_CAM;
        case VENDOR_GENETEC:    return TYPE_ALPR;
        case VENDOR_REKOR:      return TYPE_ALPR;
        case VENDOR_HIKVISION:  return TYPE_SECURITY_CAM;
        case VENDOR_DAHUA:      return TYPE_SECURITY_CAM;
        case VENDOR_AXIS:       return TYPE_SECURITY_CAM;
        case VENDOR_BOSCH:      return TYPE_SECURITY_CAM;
        case VENDOR_RING:       return TYPE_DOORBELL;
        case VENDOR_NEST:       return TYPE_SECURITY_CAM;
        case VENDOR_WYZE:       return TYPE_SECURITY_CAM;
        case VENDOR_REOLINK:    return TYPE_SECURITY_CAM;
        case VENDOR_ARLO:       return TYPE_SECURITY_CAM;
        case VENDOR_UBIQUITI:   return TYPE_SECURITY_CAM;
...
const char* alprVendorName(ALPRVendor v) {
    switch (v) {
        ...
        case VENDOR_GENETEC:    return "Genetec";
        case VENDOR_REKOR:      return "Rekor";
        case VENDOR_HIKVISION:  return "Hikvision";
        case VENDOR_DAHUA:      return "Dahua";
        case VENDOR_AXIS:       return "Axis";
        case VENDOR_BOSCH:      return "Bosch";
        case VENDOR_NEOLOGY:    return "Neology";
        case VENDOR_PERCEPTICS: return "Perceptics";
        case VENDOR_ELSAG:      return "ELSAG";
        case VENDOR_RING:       return "Ring";
        case VENDOR_NEST:       return "Nest";
        case VENDOR_WYZE:       return "Wyze";
        case VENDOR_REOLINK:    return "Reolink";
        case VENDOR_ARLO:       return "Arlo";
        case VENDOR_UBIQUITI:   return "UniFi";
...
const char* alprDeviceTypeName(ALPRDeviceType t) {
    switch (t) {
        case TYPE_ALPR:         return "ALPR";
        case TYPE_SECURITY_CAM: return "Camera";
        case TYPE_DOORBELL:     return "Doorbell";
        case TYPE_BODY_CAM:     return "BodyCam";
        case TYPE_TRAFFIC_CAM:  return "Traffic";
        default:                return "Unknown";
    }
}
...
const char* alprClassificationName(ALPRClassification c) {
    switch (c) {
        case ALPR_CLASS_DEFINITE: return "DEFINITE";
        case ALPR_CLASS_LIKELY:   return "LIKELY";
        case ALPR_CLASS_PROBABLE: return "PROBABLE";
        case ALPR_CLASS_POSSIBLE: return "POSSIBLE";
...
static void clearSeen() {
    if (!_seenTable) { _seenCount = 0; return; }
...
static int findSeen(const String &mac) {
    ...
    for (int probe = 0; probe < ALPR_SEEN_CAPACITY; probe++) {
        ...
        if (_seenTable[i].state == ALPR_SEEN_EMPTY) return -1;
...
static uint8_t calculateConfidence(uint16_t flags) {
    uint16_t score = 0;
    if (flags & ALPR_FLAG_OUI_MATCH)       score += POINTS_OUI_MATCH;
    if (flags & ALPR_FLAG_WILDCARD_PROBE)  score += POINTS_WILDCARD_PROBE;
    if (flags & ALPR_FLAG_LITEON_IE)       score += POINTS_LITEON_IE;
    if (flags & ALPR_FLAG_PACK_SIGNATURE)  score += POINTS_PACK_SIGNATURE;
    if (flags & ALPR_FLAG_BURST_PATTERN)   score += POINTS_BURST_PATTERN;
    if (flags & ALPR_FLAG_STATIONARY_RSSI) score += POINTS_STATIONARY_RSSI;
    if (flags & ALPR_FLAG_SUSTAINED_PROBE) score += POINTS_SUSTAINED_PROBE;
    if (flags & ALPR_FLAG_CHANNEL_HOPPING) score += POINTS_CHANNEL_HOPPING;
    if (flags & ALPR_FLAG_SSID_MATCH)      score += POINTS_SSID_MATCH;
    if (flags & ALPR_FLAG_BEACON_MATCH)    score += POINTS_BEACON_MATCH;
    if (flags & ALPR_FLAG_PROBE_RESP)      score += POINTS_PROBE_RESP;
    if (flags & ALPR_FLAG_BLE_XUNTONG)     score += POINTS_BLE_XUNTONG;
    if (flags & ALPR_FLAG_BLE_PENGUIN)     score += POINTS_BLE_PENGUIN;
    if (flags & ALPR_FLAG_BLE_FS_BATTERY)  score += POINTS_BLE_FS_BATTERY;
    if (flags & ALPR_FLAG_BLE_10DIGIT)     score += POINTS_BLE_10DIGIT;
    if (flags & ALPR_FLAG_BLE_NAME_MATCH)  score += POINTS_BLE_NAME_MATCH;
    return (score > 100) ? 100 : (uint8_t)score;
}

static ALPRClassification scoreToClassification(uint8_t score) {
    if (score >= ALPR_CONFIDENCE_DEFINITE) return ALPR_CLASS_DEFINITE;
    if (score >= ALPR_CONFIDENCE_LIKELY)   return ALPR_CLASS_LIKELY;
    if (score >= ALPR_CONFIDENCE_PROBABLE) return ALPR_CLASS_PROBABLE;
    if (score >= ALPR_CONFIDENCE_POSSIBLE) return ALPR_CLASS_POSSIBLE;
    return ALPR_CLASS_IGNORE;
}

static bool checkRssiStability(ALPRSeenEntry &e) {
    if (e.rssiHistoryCount < 4) return false;
    int8_t minRssi = 127, maxRssi = -127;
    for (int i = 0; i < e.rssiHistoryCount; i++) {
        if (e.rssiHistory[i] < minRssi) minRssi = e.rssiHistory[i];
        if (e.rssiHistory[i] > maxRssi) maxRssi = e.rssiHistory[i];
    }
...
static bool checkChannelHopping(ALPRSeenEntry &e) {
    int channelCount = 0;
    if (e.channelsSeen & (1 << 0)) channelCount++;
    if (e.channelsSeen & (1 << 1)) channelCount++;
    if (e.channelsSeen & (1 << 2)) channelCount++;
...
static void updateBehavioralTracking(ALPRSeenEntry &e, int8_t rssi, uint8_t channel, uint32_t now) {
    ...
    if (e.rssiHistoryCount < ALPR_RSSI_HISTORY_SIZE) e.rssiHistoryCount++;
    
    if (channel == 1)  e.channelsSeen |= (1 << 0);
    if (channel == 6)  e.channelsSeen |= (1 << 1);
    if (channel == 11) e.channelsSeen |= (1 << 2);
    ...
    if (e.burstStartTime == 0 || (now - e.burstStartTime) > ALPR_BURST_WINDOW_MS) {
        ...
        if (e.burstProbeCount < 255) e.burstProbeCount++;
    } else {
        e.burstStartTime = now;
        e.burstProbeCount = 1;
    }
...
static void updateFlags(ALPRSeenEntry &e, uint32_t now) {
    if (checkRssiStability(e))    e.flags |= ALPR_FLAG_STATIONARY_RSSI;
    else                          e.flags &= ~ALPR_FLAG_STATIONARY_RSSI;
    
    if (checkSustainedProbing(e, now)) e.flags |= ALPR_FLAG_SUSTAINED_PROBE;
    else                                e.flags &= ~ALPR_FLAG_SUSTAINED_PROBE;
    
    if (checkBurstPattern(e, now)) e.flags |= ALPR_FLAG_BURST_PATTERN;
    else                            e.flags &= ~ALPR_FLAG_BURST_PATTERN;
    
    if (checkChannelHopping(e))    e.flags |= ALPR_FLAG_CHANNEL_HOPPING;
...
static void checkAndRotateSession() {
    ...
    if (_seenCount < ALPR_SEEN_CAPACITY - 100) return;
...
static File openALPRLog() {
    ...
    if (!SD.exists("/alpr")) SD.mkdir("/alpr");
...
static String flagsToString(uint16_t flags) {
    String s = "";
    if (flags & ALPR_FLAG_OUI_MATCH)       s += "OUI|";
    if (flags & ALPR_FLAG_WILDCARD_PROBE)  s += "WILD|";
    if (flags & ALPR_FLAG_LITEON_IE)       s += "LITEON|";
    if (flags & ALPR_FLAG_PACK_SIGNATURE)  s += "PACK|";
    if (flags & ALPR_FLAG_BURST_PATTERN)   s += "BURST|";
    if (flags & ALPR_FLAG_STATIONARY_RSSI) s += "STABLE|";
    if (flags & ALPR_FLAG_SUSTAINED_PROBE) s += "SUSTAINED|";
    if (flags & ALPR_FLAG_CHANNEL_HOPPING) s += "HOP|";
    if (flags & ALPR_FLAG_SSID_MATCH)      s += "SSID|";
    if (flags & ALPR_FLAG_BEACON_MATCH)    s += "BEACON|";
    if (flags & ALPR_FLAG_PROBE_RESP)      s += "RESP|";
    if (flags & ALPR_FLAG_BLE_XUNTONG)     s += "XUNTONG|";
    if (flags & ALPR_FLAG_BLE_PENGUIN)     s += "PENGUIN|";
    if (flags & ALPR_FLAG_BLE_FS_BATTERY)  s += "FSBAT|";
    if (flags & ALPR_FLAG_BLE_10DIGIT)     s += "10DIG|";
    if (flags & ALPR_FLAG_BLE_NAME_MATCH)  s += "BLENAME|";
...
static bool IRAM_ATTR extractSSID(const uint8_t* body, int len, 
    ...
    if (!body || len < 2 || !outSsid || outLen == 0) return false;
    while (len >= 2) {
        uint8_t id = body[0];
        uint8_t elen = body[1];
        if ((int)elen + 2 > len) break;
...
static bool IRAM_ATTR hasLiteonVendorIE(const uint8_t* body, int bodyLen) {
    if (!body || bodyLen < 4) return false;
    int i = 0;
    while (i + 2 <= bodyLen) {
        uint8_t id = body[i];
        int elen = (int)body[i + 1];
        if (i + 2 + elen > bodyLen) {
            if (elen > 200) { i += 2; continue; }
            break;
        }
        i += 2;
...
static bool IRAM_ATTR hasPackSignature(const uint8_t* body, int bodyLen) {
    if (!body || bodyLen < 20) return false;
    ...
    while (i + 2 <= bodyLen) {
        ...
        switch (id) {
            case 2:   sawRates = true; break;
            case 12:  sawDS = true; break;
            case 127: sawExtCap = true; break;
            case 45:  sawHTCap = true; break;
            case 191: sawVHTCap = true; break;
            case 221:
                if (elen >= 7) {
                    if (body[i] == 0x50 && body[i+1] == 0x6f && body[i+2] == 0x9a &&
...
static void IRAM_ATTR alprWifiSniffer(void* buf, wifi_promiscuous_pkt_type_t type) {
    if (!buf || !alprDetectorActive) return;
    if (type != WIFI_PKT_MGMT) return;
    
    wifi_promiscuous_pkt_t* pkt = (wifi_promiscuous_pkt_t*)buf;
    if (pkt->rx_ctrl.sig_len < sizeof(alpr_wifi_hdr_t)) return;
    ...
    if (ouiMatch) flags |= ALPR_FLAG_OUI_MATCH;
    ...
    if (isBeacon || isProbeResponse) {
        ...
        if (isProbeResponse) flags |= ALPR_FLAG_PROBE_RESP;
    ...
    if (strlen(ssid) > 0 && isALPRSSID(ssid)) {
        ...
        if (isBeacon) flags |= ALPR_FLAG_BEACON_MATCH;
    }
    
    if (isProbeRequest && bodyLen > 0) {
        int r = isWildcardProbeIE(body, bodyLen);
        if (r == -1 && bodyLen > 4) r = isWildcardProbeIE(body, bodyLen - 4);
        if (r == 1) flags |= ALPR_FLAG_WILDCARD_PROBE;
        
        if (hasLiteonVendorIE(body, bodyLen)) flags |= ALPR_FLAG_LITEON_IE;
        if (hasPackSignature(body, bodyLen))  flags |= ALPR_FLAG_PACK_SIGNATURE;
    }
    
    if (flags == ALPR_FLAG_NONE) return;
    if (flags == ALPR_FLAG_WILDCARD_PROBE) return;
...
static void processAlert(const ALPRAlert &alert) {
    ...
    if (existing >= 0) {
        ...
        if (entry->count < 0xFFFF) entry->count++;
        ...
        if (_seenCount >= ALPR_SEEN_CAPACITY - 1) return;
    ...
    if (alert.frameSubtype == 4) entry->probeReqCount++;
    else if (alert.frameSubtype == 5) entry->probeRespCount++;
    ...
    if (isNew && entry->classification >= ALPR_CLASS_POSSIBLE) {
        ...
        worthReporting = true;
    } else if ((now - entry->lastSeen) > ALPR_REDISCOVER_MS && 
               entry->classification >= ALPR_CLASS_PROBABLE) {
        worthReporting = true;
    }
    
    if (worthReporting && oldClass != entry->classification) {
        if (oldClass == ALPR_CLASS_DEFINITE)      _countDefinite--;
        else if (oldClass == ALPR_CLASS_LIKELY)   _countLikely--;
        else if (oldClass == ALPR_CLASS_PROBABLE) _countProbable--;
        else if (oldClass == ALPR_CLASS_POSSIBLE) _countPossible--;
        
        if (entry->classification == ALPR_CLASS_DEFINITE)      _countDefinite++;
        else if (entry->classification == ALPR_CLASS_LIKELY)   _countLikely++;
        else if (entry->classification == ALPR_CLASS_PROBABLE) _countProbable++;
        else if (entry->classification == ALPR_CLASS_POSSIBLE) _countPossible++;
    }
...
static void updateChannel() {
    if (!alprDetectorActive) return;
    if (millis() - _lastHop < ALPR_CHANNEL_DWELL_MS) return;
...
void alprDetectorBegin() {
    ...
    if (!gpsActive) Serial.println("[ALPR] Warning — GPS not active.");
    if (!sdActive) Serial.println("[ALPR] Warning — SD card not mounted.");
