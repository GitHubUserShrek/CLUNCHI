...
{
    if (_justDisconnectedFlag)
    {
        _justDisconnectedFlag = false;
        return true;
    }
    return false;
}
...
void wifiBegin()
{
    if (_wifiInitialised || !WIFI_ENABLED)
        return;
...
void wifiUpdate()
{
    if (!_wifiInitialised && !scanActive)
        return;
...
{
    if (!_connected || _connEstablishedTime == 0)
        return 0;
