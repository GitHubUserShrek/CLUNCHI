...
static void parseGSV(const char *sentence, const char *talker);
...
LocalTime gpsGetLocalTime()
{
    ...
    if (gpsData.hour == 0 && gpsData.minute == 0 &&
        gpsData.second == 0 && gpsData.year == 0)
        return lt;
...
static void feedChar(char c)
{
    ...
    if (c == '\n' || c == '\r')
    {
        ...
        {
            char talker[3] = {nmeaBuf[1], nmeaBuf[2], '\0'};
            parseGSV(nmeaBuf, talker);
        }
        nmeaIdx = 0;
    }
}
...
static void parseGSV(const char *sentence, const char *talker)
{
    ...
    if (msgNum == 1 && (strcmp(talker, "GP") == 0 || strcmp(talker, "GN") == 0))
    {
        tempSatCount = 0;
        accumulating = true;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        bool dup = false;
        for (int k = 0; k < tempSatCount; k++)
        {
            if (tempSats[k].prn == prn)
            {
                dup = true;
                break;
            }
    ...
    if (msgNum >= totalMsgs)
    {
        ...
        {
            int found = -1;
            for (int k = 0; k < gpsData.satCount; k++)
            {
                if (gpsData.sats[k].prn == tempSats[i].prn)
                {
                    found = k;
                    break;
                }
            ...
            if (found >= 0)
            {
                gpsData.sats[found] = tempSats[i];
                gpsData.sats[found].lastSeen = now;
            }
            else if (gpsData.satCount < MAX_SATS)
            {
                gpsData.sats[gpsData.satCount] = tempSats[i];
                gpsData.sats[gpsData.satCount].lastSeen = now;
                gpsData.satCount++;
            }
        }

        const uint32_t SAT_TIMEOUT_MS = 5000;
        int write = 0;
        for (int read = 0; read < gpsData.satCount; read++)
        {
            if (now - gpsData.sats[read].lastSeen <= SAT_TIMEOUT_MS)
            {
                gpsData.sats[write++] = gpsData.sats[read];
            }
        }
        gpsData.satCount = write;

        for (int i = 0; i < gpsData.satCount - 1; i++)
        {
            for (int j = i + 1; j < gpsData.satCount; j++)
            {
                if (gpsData.sats[j].prn < gpsData.sats[i].prn)
                {
                    SatInfo tmp = gpsData.sats[i];
                    gpsData.sats[i] = gpsData.sats[j];
                    gpsData.sats[j] = tmp;
                }
            }
        }
